...
Dysarthria, a speech disorder that hinders speech production, can induce difficulties in patients’ daily communication. To assist dysarthric patients, voice conversion (VC) methods for dysarthric speech reconstruction (DSR) can improve intelligibility of dysarthric speech. Although successes have been made with various VC models, the identity of the dysarthric speaker is often lost during conversion. In this study, we implement a two-stage DSR framework: Stage 1 converts dysarthric speech into more intelligible, normal speech while Stage 2 preserves speaker similarity through conversion with frame-wise VC models. For Stage 1, we assessed the ability of a sequence-to-sequence model and a locally linear embedding (LLE) model to enhance dysarthric speech intelligibility with four reference speakers. Different types of content features (Chinese-HuBERT, WavLM, and Whisper) were tested for LLE conversion. For Stage 2, we surveyed a previously-tested VQ-VAE model as baseline and four state-of-the-art models (Seed-VC, FreeVC, SPARC, and MKL-VC) for the task to restore speaker identity. After evaluation, we found that VTN and LLE with Chinese-HuBERT as the content feature provided the largest improvements in intelligibility. Furthermore, we discover that Seed-VC considerably outperforms the other state-of-the-art models and the baseline in conserving speaker similarity without sacrificing intelligibility, especially when applied to VTN results.
